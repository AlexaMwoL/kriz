...
~> \`\`\`leaveall, hidetag, welcome, culik\`\`\``
               ...
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '© KRIZSER • Made By Krishnadas ☕', 
               buttons: buttons,
               headerType: 1
}
       ...
       case 'listcmd':
        ...
        case 'bc':
					...
					for (let _ of anu) {
						...
						buff = await nino.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							nino.sendMessage(_.jid, buff, video, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 101010, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Broadcast success ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await nino.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							nino.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 101010, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
...
const btnbc = {
    ...
    footerText: '*ʙʀᴏᴀᴅᴄᴀsᴛ ʙʏ _𝙺𝚁𝙸𝚉 𝚂𝙴𝚁 ♔︎_*',
    buttons: buttonss,
    headerType: 1
}
                        nino.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
await nino.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftok})
...
//------------------< Lainnya >-------------------
       ...
       case 'welcome':
              ...
              break
       case 'infoig':
              teks = `Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/kannanbro_f_f/`
              nino.sendMessage(from, teks, text, { quoted : mek })
              break
       case 'sourcecode': 
       case 'sc': 
       case 'src':
              textImg(`Bot ini menggunakan sc : ENTHADA ENNOD LUB ANO`)
              break
       case 'jadibot':
              if (!isOwner) return
